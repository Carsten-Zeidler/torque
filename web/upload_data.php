...
// Connect to Database
$con = mysqli_connect($db_host, $db_user, $db_pass) or die("DB Conenct failed");
mysqli_select_db($con, $db_name) or die("DB select failed: $db_name");

// Create an array of all the existing fields in the database
$result = mysqli_query($con, "SHOW COLUMNS FROM $db_table") or die("Get Colums failed");
...
if (sizeof($_GET) > 0) {
    ...
    foreach ($_GET as $key => $value) {
        ...
        if (!in_array($key, $dbfields) and $submitval == 1) {
            ...
            mysqli_query($con, $sqlalter) or die("Error alter table");
        }
    }
    if ((sizeof($keys) === sizeof($values)) && sizeof($keys) > 0) {
        ...
        mysqli_query($con, $sql) or die("INSERT failed");
    }
}
